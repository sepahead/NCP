...
| Script | Purpose |
|---|---|
| `check.sh [--b01-source-staging]` | Run the complete local Rust/Python/C++/TypeScript/proto/schema/profile/package preflight, including the non-authorizing B01 ADR-example profile harness. The staging flag admits only the exact zero-review superseded-packet source cut. It never authorizes release. |
| `check_implementation_ledger.py [--self-test] [--b01-source-staging]` | Validate 62 authoritative tasks and the non-authorizing B05 prototype lane. Validate B01 history, receipts, invalidations, and B03 allocations. B05 scans complete prototype and receipt trees. The scan includes inherited receipt content. It rejects unsafe paths, host paths, and credentials. The canonical runner binds exact source bytes. The runner verifies clean worktree and index state. It executes immutable gate text through explicit system Bash. It uses a minimal child environment and checked tool path. It enforces script, argument, time, output, line, and JSON limits. Timeout and overflow kill the complete process group. Portable output binds raw bytes without retaining host paths. Ignored caches and accepted user tools remain unattested local inputs. Another task must rerun checks and retain task-owned evidence. The staging flag admits only its exact B01 source cut. X05 stays `OPEN` and **NOT RUN**. |
| `run_b05_preflight.py` | Run the complete local gate for one clean B05 source cut. Use `python3 -I`. The runner emits bounded portable JSON. It grants only local prototype evidence. |
| `generate_implementation_ledger.py [--check\|--write] [--b01-source-staging]` | Generate the task-ledger view and cross-repository resumption brief from the checked JSON source. Use the staging flag only for its exact superseded-packet source cut. Do not edit the generated Markdown. |
...
| `generate_b01_review_request.py --commit COMMIT [--self-test] [--check\|--live-check\|--write]` | Generate the exact unfilled human-review roster for the immutable CURRENT B01 packet. Its check mode is an issuance-only zero-review diagnostic. Live checking reobserves canonical and local `main`. The request grants no review or protocol authority. |
| `generate_b01_reviewer_kit.py [--self-test] [--check\|--write\|--list\|--slot SLOT_ID]` | Generate the bounded non-authorizing B01 Version 2 reviewer kit. Check mode validates retained issuance lineage and the current aggregate registry. Slot output includes one domain-separated binding for all slot policy fields and the subject. The command cannot authenticate a reviewer or change the registry. |
| `preflight_b01_review_bundle.py --bundle ABSOLUTE_PRIVATE_ROOT [--self-test]` | Preflight one closed owner-mode-restricted B01 response bundle. Live bundle mode writes no application file. Self-test writes only disposable private fixtures, with bytecode disabled. The command binds exact response bytes, response semantics, the selected subject, staged evidence, tools, and current registry snapshots. It checks no extended ACL. It emits one sensitive, unauthenticated candidate. It performs no network fetch, admission, registry mutation, or authority transition. |
| `check_b01_review_handoff.py [--check] [--json] [--self-test]` | Report one read-only structural snapshot for the B01 handoff. It replays the exact registry generator. It suggests one materialization slot for each open role. The suggestion does not report slot occupancy. The command grants no identity, role, independence, history, ADR, task, protocol, or release authority. |
| `immutable_git.py` | Reopen bounded literal SHA-1 commits, trees, and blobs for B01 evidence cuts. The reader rejects replacement refs, grafts, shallow history, alternates, malformed objects, and budget exhaustion. |
| `validate_evidence_schemas.py [--self-test]` | Bounded-read and validate the ledger, decision registry, current Version 2 review chain, and handoff-status schema with the hash-locked Draft 2020-12 implementation. The same isolated lock supplies Ruff 0.15.21 for B01 Python checks. Neither tool enters a shipped package surface. Structural validation does not prove external authorship or deployment truth. |
...
| `../prototypes/b01-architecture-evidence/adr_example_semantics.py [--self-test]` | Compare separate Rust and TypeScript profile results for all 33 source-bound main-ADR JSON fences and 289 bounded mutations. The local and hosted gates pipe this result through its exact nested verifier. This is local non-authorizing evidence only. |
...
| `check_ncp_system_design_pdf.sh [--write\|--check\|--cross-toolchain]` | Convert the maintained light SVGs to vector PDF figures. Build the informative LaTeX report with fixed metadata. Bind its sources through a domain-separated SHA-256 commitment. Require one audit entry for each numbered equation. Reject layout warnings and non-embedded fonts. Same-toolchain mode requires identical PDF bytes. Cross-toolchain mode requires equal nonempty page counts, A4 geometry, and global canonical text order. It permits page-break reflow and insignificant extraction whitespace. It does not establish visual or per-page text equality. |
...
The B01 Version 2 replay binds one complete source-derived file roster.
It exact-executes captured repository helper sources and rejects unbound reads.
It still uses installed interpreter and pinned validator-package bytes.
...
| `python3 -I scripts/check_rust_packages.py` | package and inspect all Rust archives in fresh private home, Cargo-home, target, work, and temporary directories. Without a consuming-root patch, the source-only Zenoh archive probes resolve registry `zenoh-transport 1.9.0` to advisory-affected `lz4_flex 0.10.0` and its `twox-hash 1.6.3` dependency without compiling that graph. The checker applies the exact reviewed patch at each consuming test root. The conditioned graph resolves patched `lz4_flex 0.11.6` and updates its `twox-hash` dependency to `2.1.3`. It runs the verified fork's `security_backport` and compression-enabled library tests, then rechecks the source closure for both consumers. Before packaging, the checker acquires the locked workspace graph for the selected host and three exact registry inputs for normalized Zenoh graphs. It verifies the registry checksums and constructs archives offline. Conditioned lock updates run offline against acquired inputs. Later locked fetches can use network access. Cargo dependency access remains offline during compile and test. `--offline` is rejected because qualification starts with an empty source cache. The checker claims no host or child-process network isolation, host filesystem isolation, system-executable isolation, or credential-access isolation. Receipt v4 retains two conditioned locks and three checksum-bound registry crates. It records the fork verification and upstream-delta check as point-in-time local-process attestations because it does not retain the fork source bytes. It also records pre/post source comparisons for the conditioned transport closure, not a retained compiler-input trace or command transcript. The result is `CONDITIONAL_PASS`, with `package_self_contained=false`, `self_contained_distribution_gate=OPEN_FAIL_CLOSED`, `decision=NO_GO`, and `release_authorized=false`. Cargo does not verify Git signatures. |
| `python3 -I scripts/build_candidate_dossier.py --source-revision REV --output DIR` | bind a replacement-disabled archive to every blob, mode, and path in one exact Git tree; populate a fresh, config-free Cargo home with one locked fetch; build twice-compared Rust/npm/release-profile Python candidate packages; retain the observed archive fallback and conditional Zenoh-consuming-root boundary; bind the normalized regular-file-content tree of the installed TypeScript package to its reviewed Bun-lock and registry control before and after compilation; bind exact Node and npm invocations; smoke the packages; bind hashes, aggregate SBOM, and tool invocations; and emit an unsigned held dossier. The reviewed registry-tarball digest is expected control data, not build-observed derivation, because those bytes are not retained or read. The comparisons, source derivations, install checks, and probes are builder-local process attestations. The command never tags or publishes. |
| `python3 -I scripts/build_candidate_dossier.py --sdist-preflight REV` | populate a fresh Cargo home with one locked fetch, then build the exact Python sdist twice with Cargo dependency access offline; require a prune-only two-crate lock, compare archive bytes, and revalidate the extracted source under `--locked --offline` |
| `python3 -I scripts/build_candidate_dossier.py --verify-dossier DIR --require-hosted-toolchain --subject-checksums PATH` | recompute the retained dossier tree, hashes, Rust artifact controls, package/receipt consistency, toolchain policy, package subjects, and exact attestation-subject manifest without authorizing release. It does not rebuild source, rerun comparisons, or rerun Python/npm install and behavior probes. |
