...
| `check_implementation_ledger.py [--self-test] [--b01-source-staging]` | Validate the exact 62-task DAG, B01 packet lifecycle, immutable historical packet binding, hosted-CI mode parity, evidence floors, dependency receipts, reopen invalidations, bounded receipts, and B03 allocation coverage. The staging flag rejects current packets, reviews, accepted ADRs, promotion, and any N01 start. It has no X05 trust parser, so X05 stays `OPEN` and **NOT RUN**. |
