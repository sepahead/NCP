...
| `generate_b01_reviewer_kit.py [--self-test] [--check\|--write\|--list\|--slot SLOT_ID\|--materialize-response PATH]` | Generate the bounded non-authorizing B01 reviewer kit. Check mode derives the zero-review or active-capture phase and validates retained issuance lineage plus the current aggregate registry. Materialization injects machine-owned subject fields and validates a closed, untrusted envelope. The envelope binds its schema and point-in-time registry source. The command cannot authenticate a reviewer or change the registry. |
...
| `validate_evidence_schemas.py [--self-test]` | Bounded-read and validate the ledger, decision registry, reviewer kit, response schemas, and handoff-status schema with the hash-locked Draft 2020-12 implementation. The same isolated lock supplies Ruff 0.15.21 for B01 Python checks. Neither tool enters a shipped package surface. Structural validation does not prove external authorship or deployment truth. |
