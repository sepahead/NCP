...
| `check_implementation_ledger.py [--self-test]` | validate the exact 62-task DAG, B01 current-packet template and burden, evidence floors, dependency receipts, reopen invalidations, bounded receipts, and B03 allocation coverage. It rejects premature external or independent passes. It has no X05 trust parser, so X05 stays `OPEN` and **NOT RUN**. |
...
| `../prototypes/b01-architecture-evidence/adr_example_semantics.py [--self-test]` | compare separate Rust and TypeScript profile results for all 25 source-bound main-ADR JSON fences and 160 bounded mutations. The local and hosted gates pipe this result through its exact nested verifier. This is local non-authorizing evidence only. |
