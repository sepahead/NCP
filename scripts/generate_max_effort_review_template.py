...
def _source_commits(
    ...
    try:
        ...
        raise TemplateError("audit/source index does not bind both source cuts") from error
    for label, commit in (
...
def _global_findings(lenses: list[dict[str, str]]) -> list[dict[str, str]]:
    findings = {
        ...
        "L05": (
            "Typed stream positions, exact-session epochs, monotonic high-water fencing, leases, and idempotency contexts are locally tested.",
            "Crash/restart, rollover, delayed delivery, capture replay, and all-client transport lifecycle campaigns remain incomplete.",
        ),
        "L06": (
            "The candidate records compact and complete contract identities, corpus digests, profile digests, receipts, and immutable released baselines.",
            "Review evidence is unsigned, some cross-repository inputs are mutable, and complete source-to-installed-artifact lineage is not independently reproduced.",
        ),
        "L07": (
        ...
        "L09": (
            "Universal JSON limits and duplicate-key-aware ingress exist in key Rust, Python, and TypeScript paths with hostile tests.",
            "Every independent decoder and producer has not yet demonstrated identical duplicate, Unicode, non-finite, unsafe-integer, nesting, and path rejection.",
        ),
        "L10": (
        ...
        "L12": (
            "Locked dependencies, generated-output diffs, exact manifests, package extraction, and hosted CI provide strong local reproducibility controls.",
            "Signed provenance, pinned multi-platform toolchains, clean-room reproduction, and independently rebuilt final archives are absent.",
        ),
        "L13": (
        ...
        "L14": (
            "Proto-first generation, schema parity, a 269-vector corpus, shared behavior vectors, and generated TypeScript reduce projection drift.",
            "Every producer-consumer language pair, canonical byte emission path, BulkObservation surface, and independent decoder has not passed one exact matrix.",
        ),
        "L15": (
        ...
        "L17": (
            "The local gate covers Rust, Python, C/C++, TypeScript, proto/schema, corpus, profiles, packages, dependencies, and Buf, with hostile self-tests.",
        ...
        "L20": (
            "Adversarial review found a consumed-stream retry in a test, an underconstrained convergence schema, weak handoff evidence helpers, and a contradictory task DAG.",
...
def _phase_evidence(phase: str) -> tuple[list[str], list[str]]:
    mapping: list[tuple[str, list[str], list[str]]] = [
        ...
        (
            "Root protocol",
            ["README.md", "NEURO_CYBERNETIC_PROTOCOL.md", "VERSIONING.md", "RELEASE_READINESS.md"],
            ["python3 scripts/check_markdown_links.py", "scripts/check-version-coherence.sh"],
        ),
        (
            "Normative IDL",
        ...
        (
            "Contract registry",
            ["contract/manifest.v1.json", "contract/release-gates.v1.json", "contract/limits.v1.json"],
            ["python3 scripts/generate_contract_manifest.py", "python3 scripts/check_release_gates.py"],
        ),
        (
            "JSON Schemas",
            ["schemas/index.json", "schemas/README.md", "scripts/check_proto_schema_parity.py"],
            ["python3 scripts/check_proto_schema_parity.py", "python3 scripts/check_schema_defaults.py"],
        ),
        (
            "Conformance manifest",
            ["conformance/manifest.v1.json", "conformance/behavior/vectors.json", "conformance/README.md"],
            ["python3 scripts/check_conformance_vectors.py", "python3 scripts/check_behavior_vectors.py"],
        ),
        (
            "Canonical JSON",
            ["ncp-core/src/bounded_json.rs", "ncp-core/src/canonical_digest.rs", "ncp-core/tests/conformance.rs"],
            ["cargo test -p ncp-core --locked", "python3 scripts/check_request_digests.py"],
        ),
        (
            "Core message types",
            ["ncp-core/src/messages.rs", "ncp-core/src/codec.rs", "proto/ncp.proto"],
            ["cargo test -p ncp-core --locked", "python3 scripts/check_proto_schema_parity.py"],
        ),
        (
            "Sessions, generations",
            ["ncp-core/src/stream_fence.rs", "ncp-core/src/resilience.rs", "ncp-zenoh/tests/cross_session_rpc.rs"],
            ["cargo test -p ncp-core --locked", "cargo test -p ncp-zenoh --locked"],
        ),
        (
            "Authority, lease",
            ["ncp-core/src/authority.rs", "ncp-core/src/idempotency.rs", "ncp-core/src/request_digest.rs"],
            ["cargo test -p ncp-core --locked", "python3 scripts/check_request_digests.py"],
        ),
        (
            "Action safety",
            ["ncp-core/src/safety.rs", "ncp-core/src/plant.rs", "ncp-zenoh/tests/safety_governor_over_wire.rs"],
            ["cargo test -p ncp-core --locked", "cargo test -p ncp-zenoh --test safety_governor_over_wire --locked"],
        ),
        (
            "Security and plant digests",
            ["ncp-core/src/security.rs", "ncp-core/src/plant.rs", "contract/security-state-digest.v1.json"],
            ["python3 scripts/check_profile_digests.py", "python3 scripts/validate_security_profile.py --self-test"],
        ),
        (
            "Audit chain",
            ["ncp-core/src/audit.rs", "ncp-core/src/bounded_json.rs", "e2e/bounded_json.py"],
            ["cargo test -p ncp-core --locked", "python3 -m unittest -v e2e.test_bounded_json"],
        ),
        (
            "C and language bindings",
            ["ncp-cpp/include/ncp.h", "ncp-cpp/src/lib.rs", "ncp-cpp/tests/corpus.rs"],
            ["cargo test -p ncp-cpp --locked", "python3 scripts/check_behavior_vectors.py"],
        ),
        (
            "Zenoh adapter",
            ["ncp-zenoh/src/lib.rs", "ncp-zenoh/tests/loopback.rs", "ncp-zenoh/README.md"],
            ["cargo test -p ncp-zenoh --locked", "python3 scripts/check_acl_template.py"],
        ),
        (
            "Python package",
            ["ncp-python/src/lib.rs", "ncp-python/pyproject.toml", "ncp-python/tests/test_smoke.py"],
            ["python3 scripts/check_behavior_vectors.py", "python3 scripts/check_rust_packages.py --offline"],
        ),
        (
            "C++ package",
            ["ncp-cpp/include/ncp.h", "ncp-cpp/examples/demo.cpp", "ncp-cpp/tests/behavior_corpus.rs"],
            ["cargo test -p ncp-cpp --locked", "python3 scripts/check_rust_packages.py --offline"],
        ),
        (
            "TypeScript package",
            ["ncp-ts/src/index.ts", "ncp-ts/src/bounded-json.ts", "ncp-ts/scripts/check-behavior.mjs"],
            ["bun run check:behavior", "bun run check:package"],
        ),
        (
            "Gateway",
            ["ncp-gateway/src/main.rs", "ncp-gateway/README.md", "ncp-core/src/migration.rs"],
            ["cargo test -p ncp-gateway --locked", "scripts/check-version-coherence.sh"],
        ),
        (
            "Secure deployment",
            ["deploy/README.md", "deploy/zenoh-access-control.json5", "SECURITY.md"],
            ["python3 scripts/check_acl_template.py", "python3 scripts/verify_acl_deployment.py --self-test"],
        ),
        (
            "End-to-end and Engram",
            ["e2e/run_cross_language_e2e.py", "e2e/nest_five_networks.py", "e2e/README.md"],
            ["python3 -m unittest -v e2e.test_bounded_json", "python3 e2e/run_cross_language_e2e.py --help"],
        ),
        (
            "Generation and release scripts",
            ["scripts/check.sh", "scripts/README.md", "scripts/generate_contract_manifest.py"],
            ["scripts/check.sh", "python3 scripts/generate_contract_manifest.py --self-test"],
        ),
        (
        ...
        (
            ...
            ["docs/1.0-scope.md", "docs/1.0-candidate-receipts.md", "scripts/gen_diagrams.py"],
            ["python3 scripts/check_markdown_links.py", "python3 scripts/gen_diagrams.py --check"],
        ),
    ]
    for marker, paths, commands in mapping:
        if marker in phase:
            return paths, commands
    return ["RELEASE_READINESS.md", "docs/1.0-candidate-receipts.md"], ["scripts/check.sh"]
...
def _task_specific_evidence(task: dict[str, Any]) -> tuple[list[str], list[str]]:
    ...
    overrides: dict[str, tuple[list[str], list[str]]] = {
        "T125": (["README.md", "VERSIONING.md", "docs/0.8-current-baseline.md"], ["scripts/check-version-coherence.sh"]),
        "T126": (["contract/manifest.v1.json", "proto/ncp.proto"], ["python3 scripts/generate_contract_manifest.py", "scripts/check-version-coherence.sh"]),
        "T127": (["conformance/manifest.v1.json", "conformance/behavior/vectors.json"], ["python3 scripts/check_behavior_vectors.py", "bun run check:behavior"]),
        "T128": (["ncp-core/src/bounded_json.rs", "e2e/bounded_json.py", "ncp-ts/src/bounded-json.ts"], ["python3 -m unittest -v e2e.test_bounded_json", "cargo test -p ncp-core --locked"]),
        "T129": (["ncp-ts/src/bounded-json.ts", "ncp-ts/src/safety.ts"], ["bun run check:behavior"]),
        "T130": (["ncp-core/src/stream_fence.rs", "ncp-zenoh/tests/cross_session_rpc.rs"], ["cargo test -p ncp-zenoh --locked"]),
        "T131": (["e2e/run_cross_language_e2e.py", "conformance/manifest.v1.json"], ["NOT RUN: all ordered producer-consumer language pairs"]),
        "T132": (["deploy/zenoh-access-control.json5", "scripts/verify_acl_deployment.py"], ["python3 scripts/verify_acl_deployment.py --self-test", "NOT RUN: real-router identity campaign"]),
        "T133": (["README.md", "INTEGRATING.md", "docs/1.0-candidate-receipts.md"], ["NOT RUN: installed Engram native-1.0 certification"]),
        "T134": (["ncp-core/src/bulk.rs", "conformance/vectors/bulk_observation.bin"], ["cargo test -p ncp-core --locked", "NOT RUN: every stable SDK BulkObservation matrix"]),
        "T135": (["ncp-core/src/migration.rs", "INTEGRATING.md"], ["cargo test -p ncp-core --locked", "NOT RUN: legacy capture migration validator campaign"]),
        "T136": (["RELEASE_READINESS.md", "SECURITY.md"], ["NOT RUN: independent protocol, canonicalization, numeric, and deployment reviews"]),
        "T137": ([".github/workflows/ci.yml", "RELEASE_READINESS.md"], ["scripts/check.sh", "NOT RUN: full feature/platform/profile matrix"]),
        "T138": (["RELEASE_READINESS.md", "KNOWN_LIMITATIONS.md"], ["NOT RUN: duration fault, resource, cancellation, and restart campaigns"]),
        "T139": (["scripts/check_rust_packages.py", "deny.toml"], ["python3 scripts/check_rust_packages.py --offline", "cargo deny check", "NOT RUN: signed SBOM and provenance"]),
        "T140": (["RELEASE_READINESS.md", "CONTRIBUTING.md"], ["NOT RUN: independent clean-room reproduction"]),
        "T141": (["README.md", "CITATION.cff", "VERSIONING.md"], ["scripts/check-version-coherence.sh", "python3 scripts/check_markdown_links.py"]),
        "T142": ([".github/workflows/release.yml", "VERSIONING.md"], ["NOT RUN: publication, rollback, revocation, and withdrawal rehearsal"]),
        "T143": (["RELEASE_READINESS.md", "docs/handoff/max-effort-audit-inputs.v2.json"], ["python3 scripts/check_max_effort_handoff_review.py", "NOT RUN: cross-repository convergence"]),
        "T144": (["docs/handoff/max-effort-task-review.v2.json", "RELEASE_READINESS.md"], ["python3 scripts/check_max_effort_handoff_review.py --self-test", "NOT RUN: independent final lead sign-off"]),
        "T145": (["contract/release-gates.v1.json", "RELEASE_READINESS.md"], ["python3 scripts/check_release_gates.py", "NOT RUN: signed release-manager decision"]),
    }
...
def _acceptance_gap(task: dict[str, Any]) -> str:
    ...
    if number == 127:
        return "Shared decision corpora exist, but canonical emission and every mandatory vector are not proved across every independent language surface."
    if number == 128:
    ...
    if number == 129:
        return "TypeScript guards stable integer domains in key paths, but an exhaustive no-loss proof for every stable integer field is absent."
    if number == 130:
        return "Focused stream-fence and transport tests pass, but process crash, capture, gateway, and every-client restart semantics are not fully exercised."
    if number == 131:
        return "The ordered all-producer/all-consumer language-pair campaign is NOT RUN."
    if number == 132:
    ...
    if number == 134:
        return "BulkObservation is present in the wire corpus, but complete stable SDK support or explicit quarantine has not been proved."
    if number == 135:
        return "Migration helpers and prose exist, but no end-to-end capture validator proves rejection when reconstruction would require guessing."
    if number == 136:
    ...
    if number == 139:
        return "Local archives and dependency policy are checked, but final multi-package SBOM, provenance, signatures, and reproducible artifact set are absent."
    if number == 140:
    ...
    if number == 143:
        return "This local NO_GO review is not the handoff's completed convergence manifest, and cross-repository prerequisites remain unresolved."
    if number == 144:
...
def _task_record(task: dict[str, Any], comment: str | None) -> dict[str, Any]:
    ...
    return {
        ...
        "residual_risk": (
            f"Treating local coverage of {task['id']} as closure could overstate the candidate and bypass its dependency, evidence, or independent-review requirements."
        ),
...
def generate(
    ...
    if not isinstance(lenses, list) or [lens.get("id") for lens in lenses] != list(LENS_IDS):
    ...
    if unknown_comments:
        raise TemplateError(f"cannot preserve comments for unknown tasks: {sorted(unknown_comments)}")
    ...
    return {
        ...
        "requested_dispositions": {
            ...
            "doi_and_zenodo": {
                ...
                "action_taken": False,
                "finding": "No DOI or Zenodo archive is assigned or created. Citation metadata remains author-only candidate metadata until the user completes that later publication step.",
            },
...
def main() -> int:
    ...
    try:
        ...
        if args.check:
            ...
            if current != encoded:
                raise TemplateError("max-effort review differs from reproducible template")
        else:
