...
def _acceptance_gap(task: dict[str, Any]) -> str:
    ...
    if number == 143:
        return "The generated local convergence manifest records NO_GO and explicit non-local handoffs, but predecessor gates, all eleven exact consumer and extension role qualifications, and cross-repository convergence remain unresolved; its historical six-project inventory is not a role receipt."
    if number == 144:
