...
function excluded(location, key) {
    return ((location === 'root' && key === 'authority') ||
        (location === 'operation' && (key === 'request_digest' || key === 'retry')));
}
function encodeValue(writer, value, depth, location) {
    ...
    if (Array.isArray(value)) {
        ...
        for (const child of value)
            encodeValue(writer, child, depth + 1, 'nested');
        return;
    }
...
function mutationObject(request) {
    ...
    if (!['step_request', 'run_request', 'close_session'].includes(String(object.kind))) {
        throw new RequestDigestError(`request kind ${JSON.stringify(object.kind)} is not a state-mutating lifecycle request`);
    }
...
export function canonicalRequestProjection(request) {
...
export function requestDigest(request) {
    return Array.from(sha256(canonicalRequestProjection(request)), (byte) => byte.toString(16).padStart(2, '0')).join('');
}
/** Verify the embedded operation.request_digest. */
export function verifyRequestDigest(request) {
    const object = mutationObject(request);
