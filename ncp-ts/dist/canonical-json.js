...
function member(value, key, fallback = MISSING) {
...
function array(value, path, encode) {
    if (!Array.isArray(value))
        throw new Error(`${path} must be an array`);
    return `[${value.map((item, index) => encode(item, `${path}[${index}]`)).join(',')}]`;
}
function map(value, path, encode) {
    const entries = Object.entries(record(value, path)).sort(([left], [right]) => utf8Compare(left, right));
    return object(entries.map(([key, item]) => [key, encode(item, `${path}[${JSON.stringify(key)}]`)]));
}
...
function number(value, path) {
    ...
    if (magnitude < 1e-5 || magnitude >= 1e16) {
        return value
            .toExponential()
            .replace(/e([+-]?)0+(\d+)/u, (_match, sign, exponent) => `e${sign}${exponent}`);
    }
    const rendered = value.toString();
    return Number.isInteger(value) ? `${rendered}.0` : rendered;
}
function nullable(value, path, encode) {
    return value === null ? 'null' : encode(value, path);
}
const strings = (value, path) => array(value, path, string);
const integers = (value, path) => array(value, path, integer);
const numbers = (value, path) => array(value, path, number);
function sessionRef(value, path) {
    const input = record(value, path);
    return object([['generation', string(member(input, 'generation'), `${path}.generation`)]]);
}
function streamPosition(value, path) {
    const input = record(value, path);
    return object([
...
function identityClaim(value, path) {
    const input = record(value, path);
    return object([
...
function gatewayAttribution(value, path) {
    const input = record(value, path);
    return object([
...
function authorityLease(value, path) {
    const input = record(value, path);
    return object([
...
function operationContext(value, path) {
    const input = record(value, path);
    return object([
...
function responderReceipt(value, path) {
    const input = record(value, path);
    return object([
        ['operation_id', string(member(input, 'operation_id'), `${path}.operation_id`)],
        ['request_digest', string(member(input, 'request_digest'), `${path}.request_digest`)],
...
function channelValue(value, path) {
    const input = record(value, path);
    return object([
...
function channelMap(value, path) {
...
function countStreamPosition(counter, value, path) {
    const input = record(value, path);
    counter.object([
...
function countSessionRef(counter, value, path) {
    const input = record(value, path);
    counter.object([
        ['generation', () => counter.jsonString(member(input, 'generation'), `${path}.generation`)],
    ]);
}
function countAuthorityLease(counter, value, path) {
    const input = record(value, path);
    counter.object([
...
function countChannelMap(counter, value, path) {
    const input = record(value, path);
    counter.ascii('{');
    let index = 0;
    for (const key in input) {
        ...
        counter.object([
            [
                'data',
                () => counter.array(member(channel, 'data', []), `${path}[${JSON.stringify(key)}].data`, (item, itemPath) => counter.number(item, itemPath)),
            ],
            [
                'unit',
                () => {
                    ...
                    if (unit === null)
                        ...
                        counter.jsonString(unit, `${path}[${JSON.stringify(key)}].unit`);
                },
            ],
        ]);
    }
...
export function canonicalDataPlaneByteLength(value, expectedKind, maximum) {
    ...
    counter.object([
        ...
        [
            'horizon',
            () => counter.array(member(input, 'horizon', []), 'command_frame.horizon', (step, path) => countChannelMap(counter, step, path)),
        ],
        [
            'horizon_dt_ms',
            () => {
                ...
                if (value === null)
                    ...
                    counter.number(value, 'command_frame.horizon_dt_ms');
            },
        ],
        ...
        [
            ...
            () => {
                ...
                if (source === null)
                    ...
                    countStreamPosition(counter, source, 'command_frame.source');
            },
        ],
        ...
        [
            ...
            () => {
                ...
                if (authority === null)
                    ...
                    countAuthorityLease(counter, authority, 'command_frame.authority');
            },
        ],
    ]);
    return counter.bytes;
}
function networkRef(value, path) {
    const input = record(value, path);
    return object([
...
function recordTarget(value, path) {
    const input = record(value, path);
    return object([
...
function recordSpec(value, path) {
    const input = record(value, path);
    return object([
        ['targets', array(member(input, 'targets', []), `${path}.targets`, recordTarget)],
    ]);
}
function stimulusTarget(value, path) {
    const input = record(value, path);
    return object([
        ['port', string(member(input, 'port'), `${path}.port`)],
        ['target', string(member(input, 'target'), `${path}.target`)],
...
function entityBinding(value, path) {
    const input = record(value, path);
    return object([
...
function simProvenance(value, path) {
    const input = record(value, path);
    return object([
...
function channelSpec(value, path) {
    const input = record(value, path);
    return object([
...
function safetyLimits(value, path) {
    const input = record(value, path);
    return object([
...
function stimulusFrame(value, path) {
    const input = record(value, path);
    return object([
...
export function canonicalizeNcpMessage(value, expectedKind) {
...
export function canonicalizeNcpJson(input, expectedKind) {
