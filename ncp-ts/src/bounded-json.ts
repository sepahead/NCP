...
function jsonIntegerMagnitudeStart(token: string): number | undefined {
  ...
  if (first === 0x30) return token.length - start === 1 ? start : undefined
  if (first < 0x31 || first > 0x39) return undefined
...
function integerTokenExceedsSafeRange(token: string): boolean {
  const magnitudeStart = jsonIntegerMagnitudeStart(token)
  if (magnitudeStart === undefined) return false
  ...
  for (let index = 0; index < magnitudeLength; index++) {
    ...
    if (digit !== boundaryDigit) return digit > boundaryDigit
  }
  return false
}

export type JsonLimitCode =
  | 'NCP-LIMIT-001'
  | 'NCP-LIMIT-002'
  | 'NCP-LIMIT-003'
  | 'NCP-LIMIT-004'
 ...
 */
function utf8ByteLength(input: string, end = input.length, stopAfter = Number.MAX_SAFE_INTEGER): number {
  let bytes = 0
...
class Scanner {
  ...
  private fail(code: JsonLimitCode, detail: string): never {
  ...
  private expect(expected: string): void {
    if (this.input[this.position] !== expected) this.fail('NCP-LIMIT-009', 'unexpected token')
    this.position++
  }
  ...
  scan(): void {
    this.parseValue(0)
    this.skipWhitespace()
    if (this.position !== this.input.length) this.fail('NCP-LIMIT-009', 'trailing JSON bytes')
  }
  ...
  private parseNumber(): void {
    ...
    while (/[0-9+\-.eE]/u.test(this.input[this.position] ?? '')) this.position++
  ...
  private parseString(key: boolean): string {
    const start = this.position
    this.expect('"')
    let escaped = false
    for (;;) {
      const character = this.input[this.position]
      if (character === undefined) this.fail('NCP-LIMIT-009', 'unterminated JSON string')
      this.position++
      if (escaped) {
        escaped = false
        continue
    ...
    if (typeof decoded !== 'string') this.fail('NCP-LIMIT-008', 'invalid JSON string')
    ...
    if (bytes > limit) this.fail('NCP-LIMIT-005', 'JSON string exceeds byte limit')
  ...
  private parseObject(depth: number): void {
    if (depth > JSON_LIMITS.maxNestingDepth) this.fail('NCP-LIMIT-002', 'JSON nesting depth exceeded')
    this.objects++
    if (this.objects > JSON_LIMITS.maxObjects) this.fail('NCP-LIMIT-003', 'object count exceeded')
    this.expect('{')
    this.skipWhitespace()
    ...
    for (;;) {
      ...
      if (keys.has(key)) this.fail('NCP-LIMIT-007', 'duplicate JSON object key')
      keys.add(key)
      this.members++
      if (
        keys.size > JSON_LIMITS.maxObjectMembers ||
  ...
  private parseArray(depth: number): void {
    if (depth > JSON_LIMITS.maxNestingDepth) this.fail('NCP-LIMIT-002', 'JSON nesting depth exceeded')
    this.arrays++
    if (this.arrays > JSON_LIMITS.maxArrays) this.fail('NCP-LIMIT-004', 'array count exceeded')
    this.expect('[')
    this.skipWhitespace()
    if (this.input[this.position] === ']') {
      this.position++
    ...
    for (;;) {
      ...
      this.arrayItems++
      if (
        localItems > JSON_LIMITS.maxArrayItems ||
