...
ADR-003, ADR-004, ADR-005, ADR-006, ADR-007, ADR-010, ADR-011, D18 through D21, and consumer scopes changed after cross-repository review. Earlier requests, model results, and gates bind superseded bytes only. The review packet is CURRENT. It binds source commit adfb064d901cc8ae459dbddd5d7b26cda7ba02c0 and tree 21b1e14fe6cb80950d251a68347c4007b67d6410. The current proposed decision set 1113dd7ca5a333a3f0a91632b8d8c52f8ac576045f959830434ba74067953bc4 has zero reviews. The retained request exposes all 53 unfilled identity slots and 112 evidence requirements without supplying identities or decisions. The reviewer kit adds bounded slot discovery, retained-issuance validation, aggregate-registry replay, and private preflight without changing that authority boundary. Composite local runs covered every command in `scripts/check.sh`. One uninterrupted attempt stopped only when crates.io timed out during the Python source-distribution build. The exact failed step and remaining suffix then passed separately. The exact clean B01 runner passed. The current bounded 33-case, 289-mutation semantic replay passes in separate Rust and TypeScript engines. The selector authoring artifact retains a superseded pre-atomic-HOLD allocation as an INCOMPLETE_FAIL_CLOSED and NOT_REVIEWED diagnostic outside the semantic-closure subject. It cannot accept an ADR or replace required human review. These local results do not satisfy B01's independent evidence floor and are not passing task evidence. The Ed25519 screen uses a fixed-sample p95 computational tripwire of 100,000 microseconds for thread and process CPU. Maximum CPU and wall time remain observational. This coordination entry has no passing task-source receipt, authenticated owner or independent review, command receipt, or artifact receipt. All ADRs remain PROPOSED, and B01 remains IN_PROGRESS. Use the retained request for qualifying human reviews and external adjudications. Do not create the normative registry, start descendants, or infer release readiness.
...
- Decision set 1113dd7ca5a333a3f0a91632b8d8c52f8ac576045f959830434ba74067953bc4 has zero review records. All 52 obligations and 53 minimum identity slots remain unfilled. The request and kit expose 112 evidence requirements with machine-owned subjects only. The lifecycle gate preserves immutable issuance and replays the current aggregate with exact generator bytes.
- Private-bundle preflight emits a structurally validated, unauthenticated stdout candidate only. No owner or independent review, adjudication, or passing B01 receipt exists.
- Phase derivation is a current-snapshot structural check. It does not prove append-only history or detect removal, rollback, replay, or equivocation. External verification and adjudication must preserve an authenticated high-water mark.
...
- Idempotency lookup revalidates the expired request deadline and live lease before cache access, so a known terminal result can become unavailable. Zenoh now consumes a fresh sequence before serialization and does not reuse a displaced pending command's position. These local guards do not implement the selected prepared-publication and no-reuse architecture. B02, B03, N03, N05, N06, and N07 remain open.
...
- The TypeScript client and gateway immediately remove a live local generation when open or close starts. This blocks new mutation work, but it also discards recovery coordinates before a definitive backend result. A timeout, rejection, or superseded successful open can leave an unresolved server generation without retained local lineage. The selected session mutation owner keeps that lineage fenced under the same operation until reconciliation. B02, B03, N02, N03, N06, and N07 remain open.
- NeuroControlLoop stores a host-supplied serialized AuthorityLease and checks only its shape and session equality before Active output. It has no receiver-owned monotonic lease deadline or opaque live-authority capability. Failure to clear the host value can therefore preserve apparent authority indefinitely. The selected body owner must enforce the current local deadline and capability. B02, B03, N03, N05, N07, and N08 remain open.
- NeuroControlLoop::tick returns an unadmitted pre-sensor fallback in the same CommandFrame type as a transport-admitted command. CommandSendOutcome binds only a position and cannot prove which exact bytes the transport retained. The selected production handoff needs a closed prepared-versus-admitted result and an exact owner-incarnation, position, and payload-digest binding. B02, B03, N03, N05, and N07 remain open.
- The experimental TypeScript WebSocket binding requires replies in request order and uses FIFO correlation. Malformed, unsolicited, or binary replies retire the transport, as do exchange timeouts. It does not implement exact response-identity correlation for multiplexed or out-of-order peers. B02, B03, N03, N06, and N07 remain open.
...
- The experimental WebSocket binding bounds 128 requests and 8 MiB of retained serialized payloads. Payloads are released after WebSocket.send returns, excluding browser and kernel buffers. Zenoh has bounded queues, frame limits, and a saturating drop counter. It lacks an independent aggregate byte limit and session-wide subscriber/worker ceilings. Grant-bound compact numeric projection and NCPB transport identity/provenance remain absent. B02, B03, N01, N03, N05, N06, and N07 remain open.
- BulkBlock remains a local or offline helper. Encoding uses one final byte vector and borrowed name indexes. Decoding borrows names during uniqueness checks and allocates owned columns only after validating the complete directory. The 64 MiB block ceiling does not establish measured peak memory or an authenticated one-buffer observer transport. B02, B03, N01, N06, and N07 remain open.
...
passes ordinary stream, replay, lease, freshness, declaration, source, channel, and
profile admission. Its reservation atomically binds the admitted predecessor and the
exact `CLEAR_ACTIVE` intent. `NONE_ACTIVE` forbids a reservation. Each qualified
effect requires an exact durable `BodyFailSafeSideEffectReservation` before the local
effect. Attempt identity is not command identity.
...
ESTOP that passes the complete pre-replay gate can latch and then receive stream-order,
occupied-position, command-identity, or post-boundary currentness/deadline rejection.
Zero position, foreign epoch, or another pre-replay failure cannot latch. The
equivalent invalid HOLD has no remote side effect and uses
`received` to `rejected`
