...
B04 checkpoint 2026-07-16T10:53:53Z: the Zenoh metadata-probe section is pushed, clean, and remote-equal at `b1dcf1bf388f8223b4117fcd7b5dbb9f190286b0`. The isolated crate is pinned to crates.io `zenoh 1.9.0`, checksum `85e22d7002ac149ef17fe400bb40a267ebbba40a83413bab03da7762256fa94e`, and upstream `81c6c933b6e41d72a05f04c4442ef57717ddc72b`. Eleven source files establish the callback boundary; the live probe shows publisher, query, and reply senders choosing `SourceInfo` IDs and sequences, omission remaining absent, and source-less liveliness. Seven hostile verifier tests, warnings-denied clippy, one live Rust test, result comparison, prose/link checks, and 1440/390 light/dark visual QA pass. The section review rejects overclaiming: live behavior cannot prove API absence, Zenoh IDs are not certificate principals, the same-process run is not interoperability, direct Zenoh `production-secure` remains unavailable, and `RUSTSEC-2026-0041` in the compression-disabled graph is not waived for shipping. All three lenses pass only for this prerequisite. B04 remains IN_PROGRESS; external gates remain NOT RUN. On resume verify this coordination checkpoint is remote-equal if pushed, then implement the smallest isolated terminating TLS 1.3 ingress prototype A from `docs/research/authenticated-ingress-feasibility.md`: verified client-certificate principal at the owning boundary, manifest admission distinct from authority, pre-allocation framing limits, protected local handoff, replay recovery, hostile certificate/parser cases, resource measurements, and no NCP parsing before authentication/admission. Explain and independently challenge A, perform its three-lens review, update this ledger, and commit/push before considering forwarding prototype B. Do not alter the normative wire, root workspace, shipping packages, direct-Zenoh fail-closed behavior, or a consumer repository from this checkpoint.

Current residual risks:

- The exact Zenoh 1.9.0 source/API matrix, live sender-control counterexamples, hostile verifier cases, deterministic result, and section review now have a retained local pass, but no authenticated-ingress implementation, independent-parser result, ingress resource measurement, or terminal B04 feasibility result exists yet.
