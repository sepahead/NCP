...
B04 checkpoint 2026-07-16T10:11:59Z: the design section is committed, pushed, clean, and remote-equal at `18a7bca2bb2736edb61fac1d3b57e30c443c7b05`. It passed its first-principles protocol/security/plant, consumer/runtime, and operations/science/evidence review; this proves a bounded plan only. The exact-Zenoh probe section now starts. Build an isolated, non-workspace, non-shipping crate pinned to `zenoh = 1.9.0` and the locked crates.io checksum. Retain a source/API matrix and executable observations for stable subscriber metadata, sender-chosen unstable `SourceInfo`, query source/replier data, session peer IDs, and source-less liveliness. The honest conclusion must combine pinned-source inspection with observed behavior; a live probe alone cannot prove API absence or transport security. On resume verify this start checkpoint is remote-equal if already pushed, then implement the smallest source-bound probe, lock its dependencies, run positive and hostile cases, record exact tool/environment/digests, explain the result, and perform the three-perspective section review before starting A. Do not add the crate to shipping packages or the stable workspace and do not infer certificate-principal binding, authorization, interoperability, or any external gate.
