...
B04 checkpoint 2026-07-16T13:39:49Z: terminating TLS-ingress prototype A is pushed and remote-equal at `08be80eb98da5cd6e60af457c94d1c33e6890343`, tree `99ea19d7f5f4f3d86f79586b64524949e3c7cd5a`. The isolated unpublished crate pins rustls 0.23.40 and tokio-rustls 0.26.4 to ring/std without TLS 1.2, tickets, resumption, early data, logging, or secret extraction. It terminates loopback TLS 1.3 mTLS, hashes the exact verified leaf DER, performs deny-only early lookup, bounds framing before allocation, validates the same NCP bytes, and makes one final certificate/grant/identity decision against an immutable current manifest snapshot. Its safe-Rust handoff prevents accidental construction, cloning, serialization, mutation, detachment, and payload debug disclosure; it grants no authority. Eight hostile verifier tests, eighteen Rust tests, strict clippy, docs, external OpenSSL negatives, exact live-result verification, ledger/link checks, and a zero-vulnerability local RustSec scan of 90 lockfile dependencies pass. Five exact Fable consultations were non-normative assumption challenges, not review or evidence. Limits remain: absent EKU is accepted by pinned webpki, manifest publication is unauthenticated, generation and boot state are process-local, revocation is instantaneous rather than continuing, and the volatile duplicate observer is not replay protection. Prototype A passes only its bounded local lenses; B04 remains IN_PROGRESS and external gates remain NOT RUN. On resume verify this receipt commit remotely, then implement the quarantined strict signed-forwarding prototype B with durable atomic replay semantics before two independent non-Rust parsers. Do not change the wire, root workspace, shipping packages, direct-Zenoh fail-closed behavior, or consumer repositories.

Current residual risks:

- The exact Zenoh 1.9.0 metadata boundary and isolated terminating TLS 1.3 ingress prototype A now have retained local section passes, but no signed-forwarding result, independent-parser result, or terminal B04 feasibility result exists yet.
...
- Ingress co-tenancy, authenticated manifest provenance, restart-spanning rollback protection, continuing revocation, durable replay recovery authority, exact forwarding congruence, and crypto/parser dependency independence remain unproved prototype falsification targets.
