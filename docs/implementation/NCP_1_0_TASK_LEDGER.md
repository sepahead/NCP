...
- Checked codec paths can invent midpoint or zero values, accept sparse components, and select a unit by mapping order. The plant helper is not integrated into Active admission, and `PlantCommand` erases units. `PlantProfile::validate_active_command` also revalidates, hashes, and rebuilds a name set per call. ADR-007 must reconcile the selected ESTOP-only early latch and ordinary HOLD admission. B01, B02, B03, N01, N03, N05, N07, N08, E04, H02, and C02 remain open.
...
- The current runtime has no single body-generation owner, fixed cut and emergency cells, bounded normal-command fairness, receiver-owned single-use executor capability, or deployment-fenced owner incarnation. A restart or process handoff therefore has no selected actuator-capable split-brain boundary. B01 must reconcile the semantic requirement. B03 and dependency-ready implementation tasks must select and implement the exact local or cross-process handoff.
- The runtime lacks three selected cross-boundary orders. They cover durable outbox send attempts, observer release versus revocation, and effect-path reservation before plant opening. Without them, crash ambiguity can duplicate mutation, revoked bytes can escape, and separate realms can target the same hardware. B01 must reconcile them before B03 and dependency-ready implementation work select exact profiles.
...
- ADR-007 orders a remote HOLD side effect before stream replay and the live-holder lease check. The selected target reserves that bypass for ESTOP. The current command also carries source and remote values for restrictive modes that must use installed local actions only. B01 must reconcile the mode and ordering before B02, B03, N03, N05, N07, and N08 implement them.
...
- The selected early ESTOP latch needs a bounded replay identity when its bytes conflict with an occupied command position. The corrected target keeps the primary digest immutable and preallocates one generation-wide restrictive-conflict attribution beside the one-transition latch. The current runtime has neither record. B01 must reconcile it before B03, N03, N05, N07, and N08 implement command admission.
- The selected Active path requires exact retained source-publication evidence. The current runtime has no bounded body-owned source window for the plant-profile safety projection. It also cannot install that projection in a grant-reserved pin. A latest sensor slot, bare source position, timestamp, or digest without available values is insufficient. B01 must define finite pin, eviction, expiry, and missing-evidence behavior. B03, N03, N05, N07, and N08 remain blocked from command admission.
- ADR-008 bounds protected extension bytes but does not yet select an outer transport that composes with the universal frame limit. Reassembly capacity alone also does not reserve a concrete bounded schema arena and callback slot. B01 must close the outer wrapper and semantic-allocation behavior before B03, N03, N06, G01, H02, and C03 implement extension delivery.
- The current Rust and TypeScript handshake paths accept both 1 and 1.0 plus any parseable 1.x wire. They treat an absent or different compact proto hash as advisory. They do not enforce the exact supported wire literal plus stable-core identity selected by the low-overhead architecture. B01 must reconcile that identity rule. B02, B03, N01, N03, N05, N06, and N07 remain blocked from implementation and qualification.
- Rust message structs and the generic validator accept unknown members within a compatible major, then typed conversion discards them. A signer, replay digest, gateway, and typed receiver can therefore observe different objects. The selected exact native profile requires closed stable message shapes and explicit bounded evolution points. B01 must reconcile this before B02, B03, N01, N04, N05, N06, N07, and N08 implement or qualify admission.
...
- Simulation also lacks a pre-reserved request and response window, contiguous range allocator, strict execution cursor, and bounded non-refreshing gap deadline. It has no digest-bound retained response map, position correlation, or terminable step boundary. B01 must reconcile these semantics before B02, B03, N02, N03, N07, E02, and E03 implement them.
...
- The local ActionBuffer latches ESTOP before complete wire validation. ActionBuffer and the stream fence also adopt the first admitted epoch instead of comparing with a prepared declaration. The fence hashes variable route, kind, and generation strings for every frame instead of using a prepared fixed slot. LinkMonitor permits a CUSUM threshold above its 256-sample work bound, normalizes invalid parameters, and exposes no counter-saturation state.
...
| `B01` | `IN_PROGRESS` | `COORDINATION_ONLY` | `INDEPENDENT` | `B01-acceptance`, `D01`, `D02`, `D03`, `D04`, `D05`, `D06`, `D07`, `D08`, `D09`, `D10`, `D11`, `D12`, `D13`, `D14`, `D15`, `D16`, `D17`, `D18`, `D19`, `D20` | Decide and ratify ADR-001 through ADR-011 | `B04` | NCP | `—` | 42 |
