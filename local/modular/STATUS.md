...
| Prisoma Agent Bridge | Canonical command and response records joined to sensor execution and captured exchanges. | Independent future-outcome labels or completed model-quality studies. |
...
The current sensor interface excludes privileged CPU checkpoints and counterfactual records from ordinary observation payloads.
Native CREBAIN fork support does not create an NCP predictor-access or restored-label contract by itself.
A complete world-model experiment must join forecasts, selected execution, independent branch outcomes, and the frozen comparison rule.
The existing canonical sensor execution path establishes only its declared part of that sequence.
...
- [Engram coupling policy (private source)](https://github.com/sepahead/Paper2Brain/blob/main/packages/ncp-nest/examples/crebain_coupling.py), requiring private repository access.
...
| Canonical sensor execution | Prisoma Agent Bridge joins commands, receipts, transcript spans, and original sensor bytes. | One native case reconstructed eight commands, 150 exchanges, and 1,542,400 sensor bytes. | Current installed composition, forecasts, and restored branch labels require separate evidence. |

Published application records retain the source and runtime identities for each row.
...
| Qualify advertised optional combinations | Installation together does not prove independence from unselected packages. The latest body campaign used transcript capture, without Agent Bridge. | Clean documented installations and native runs for each advertised combination, including canonical experiment execution. |
| Measure the admitted resource envelope | The selected M1 maximum due batch is 385,072 bytes, while source admission permits 27,857,088 bytes. | Frozen workload sizes, completed bytes, failures, timing, and process/graphics resource observations. |
| Extend the modular body to many entities | The current force-ground sensor application admits one drone and no city solids. | A declared many-entity application contract and native action/sensor routing controls. |
| Complete the embodied world-model path | Sensor execution does not provide the complete forecast and independently restored-label workflow. | Prisoma-owned candidate, forecast, execution, label, comparison, and replay evidence. |
