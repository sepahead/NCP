...
C/C++) with field-set-parity drift guards, not yet a multi-implementation program. It is **pre-1.0** (current wire `0.2`, released as `v0.2.0`/`v0.2.1`/`v0.2.2`/`v0.2.3`): the wire
...
- **Zenodo DOI via the GitHub–Zenodo archive.** Tagged releases now exist
  (`v0.2.0`/`v0.2.1`/`v0.2.2`/`v0.2.3`); enable the GitHub–Zenodo integration so a release is archived
