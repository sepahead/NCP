...
| **Cross-language** | a **Rust** client ([`../ncp-core/examples/ncp_tcp_client.rs`](../ncp-core/examples/ncp_tcp_client.rs)) drives the **Python** engram server over the wire (a crebain/prisoma peer ↔ engram), contract verified | `run_cross_language_e2e.py` (this dir) | local (needs both repos) |
