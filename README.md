...
|---|---|---|---|
| **`ncp-core`** (Rust) | `ncp-core = { git = "https://github.com/sepahead/NCP", tag = "v0.4.1" }` | Build `OpenSession` / `CommandFrame`, `serde_json::to_string` → wire — see [`ncp-core/README.md`](ncp-core/README.md) | none (serde-only; in-process bus + control loop) |
| **`ncp-zenoh`** (Rust transport) | `ncp-zenoh = { git = "https://github.com/sepahead/NCP", tag = "v0.4.1" }` | `let bus = ZenohBus::open().await?; let client = ZenohNcpClient::new(bus); client.open(&msg).await?` — see [`ncp-zenoh/README.md`](ncp-zenoh/README.md) | Zenoh (queryable RPC + per-plane pub/sub) |
...
```toml
[dependencies]
ncp-core  = { git = "https://github.com/sepahead/NCP", tag = "v0.4.1" }
ncp-zenoh = { git = "https://github.com/sepahead/NCP", tag = "v0.4.1" }  # transport, optional
```
...
- [`CHANGELOG.md`](CHANGELOG.md) — per-release notes (current: `v0.4.1`).

## Status

NCP is **pre-1.0 and experimental.** Specifically:

- **The wire may change.** Minor versions are treated as breaking; the version guard fails closed rather than coercing. **Pin the latest tag** (`tag = "v0.4.1"` above — the wire is `0.4`, with `v0.4.1` the buf-breaking baseline) for anything you build against.
