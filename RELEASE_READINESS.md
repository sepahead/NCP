...
Status of NCP's wire-0.7 **`v0.7.0` release**: can the `v0.7.x` line evolve
...
**Verdict (wire `0.7`, released as `v0.7.0`; checklist re-audited from the `v0.5.0`
...
pass, consumers are coordinated, and `v0.7.0` is the latest immutable tag.
...
- **Pre-1.0 (`0.x`).** The wire may still change; minor-is-breaking, the version guard
  fails closed. Pin the latest immutable release, `tag = "v0.7.0"`.
...
release gates make `v0.7.0` the immutable wire-0.7 release.
